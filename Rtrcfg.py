...
class Router(object):
    ...
    def __init__(self, ip, user, passwd, cmdfile):
        self.ip = ip.strip()
    ...
    def __ssh__(self):
        ...
        try:
            ...
            self.__ssh_client__.connect(self.ip, username=self.username, password=self.password, look_for_keys=False, allow_agent=False)
...
def create_dev(ip):
    dev = Router(ip, username, password, cmd_file)
    if dev.err:
